 ...
 * @LastEditors: ldx
 * @LastEditTime: 2023-12-17 12:27:53
 */
...
const mapList: TileType[] = [
  ...
  },
  {
    ...
    urls: [
      ...
      'https://wprd0{1-4}.is.autonavi.com/appmaptile?lang=zh_cn&size=1&style=8&x={x}&y={y}&z={z}&scl=1&ltype=4'
    ]
  },
  {
    name: '百度地图',
    value: 'baidu',
    urls: [
      'https://maponline2.bdimg.com/tile/?qt=vtile&x={x}&y={y}&z={z}&styles=pl&scaler=2&udt=&from=jsapi2_0'
    ],
    origin: 'center', // 坐标系原点
    ...
    mercatorToLngLat(lng: number, lat: number) {
      return gcoord.transform([lng, lat], gcoord.BD09MC, gcoord.GCJ02)
    }
  },
  {
    name: '腾讯地图',
    value: 'tx',
    urls: [
      'https://rt{1-3}.map.gtimg.com/tile?z={z}&x={x}&y={y}&styleid=1&scene=0'
    ],
  ...
  },
  {
    ...
    value: 'tianditu',
    urls: [
      'http://t{0-7}.tianditu.com/DataServer?T=vec_w&tk=2c009e76130364aaf09aa30ef0621154&x={x}&y={y}&l={z}',
      'http://t3.tianditu.com/DataServer?T=cva_w&tk=2c009e76130364aaf09aa30ef0621154&x={x}&y={y}&l={z}'
    ],
    // 需要火星坐标系转4326
  ...
  },
  {
    ...
    getTileUrl(x: number, y: number, z: number) {
      ...
      const domainIndex = getRandomDomainIndex(
        'http://dynamic.t{0-3}.tiles.ditu.live.com/comp/ch/'
      )
...
class TileLayer extends Group {
  ...
  get width() {
    ...
    if (!layer) return 0
    const { viewportWidth } = layer?.getViewPort()
    return viewportWidth
  }
  get height() {
    const layer = this.getLayer()
    if (!layer) return 0
    const { viewportHeight } = layer?.getViewPort()
  ...
  async renderTiles(isFadeIn = false) {
    ...
    // 中心点对应的瓦片
    const centerTile = getTileRowAndCol(...center, this.zoom, plusOpt)
    // 中心瓦片左上角对应的像素坐标
    const centerTilePos = [centerTile[0] * TILE_SIZE, centerTile[1] * TILE_SIZE]
    // 中心点对应的像素坐标
    const centerPos = getPxFromLngLat(...center, this.zoom, plusOpt)
    ...
    // 计算瓦片数量
    const rowMinNum = Math.ceil((this.width / 2 - offset[0]) / TILE_SIZE)
    const colMinNum = Math.ceil((this.height / 2 - offset[1]) / TILE_SIZE)
    const rowMaxNum = Math.ceil(
      (this.width / 2 - (TILE_SIZE - offset[0])) / TILE_SIZE
    )
    const colMaxNum = Math.ceil(
      (this.height / 2 - (TILE_SIZE - offset[1])) / TILE_SIZE
    )
    // y轴向上
    const axisYIsTop = this.tile.axis ? this.tile.axis[1] === 'top' : false
    this.currentTileCache = {} // 清空缓存对象
    // 渲染画布内所有瓦片
    for (let i = -rowMinNum; i <= rowMaxNum; i++) {
      for (let j = -colMinNum; j <= colMaxNum; j++) {
        // 当前瓦片的行列号
        const row = centerTile[0] + i
        const col = centerTile[1] + j
        // 当前瓦片的显示位置
        let _j = j
        // 百度地图，坐标系和画布坐标系y轴相反
        if (axisYIsTop && j !== 0) {
        ...
        const y =
          _j * TILE_SIZE - (axisYIsTop ? TILE_SIZE - offset[1] : offset[1])
        // 缓存key
        const cacheKey = row + '_' + col + '_' + this.zoom
        // 记录当前需要的瓦片
